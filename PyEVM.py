...
def build_laplacian_pyramid(src,levels=3):
    ...
    for i in range(levels,0,-1):
        ...
        GE = cv2.resize(GE, dsize=(gaussianPyramid[i-1].shape[1], gaussianPyramid[i-1].shape[0]))
...
def reconstract_video(amp_video,origin_video,levels=3):
    ...
    for i in range(0,amp_video.shape[0]):
        ...
        for x in range(levels):
            img=cv2.pyrUp(img)
        img_resized = cv2.resize(img, (origin_video[i].shape[1], origin_video[i].shape[0]))
        img=img_resized+origin_video[i]
        final_video[i]=img
    return final_video

#save video to files
def save_video(video_tensor,outputpath="out.avi"):
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    [height,width]=video_tensor[0].shape[0:2]
    writer = cv2.VideoWriter(outputpath, fourcc, 20, (width, height), 1)
    for i in range(0,video_tensor.shape[0]):
        writer.write(cv2.convertScaleAbs(video_tensor[i]))
    writer.release()

#magnify color
def magnify_color(video_name,outputpath,low,high,levels=3,amplification=280):
    print("load video")
    start=time.time()
...
#manify motion
def magnify_motion(video_name,ouputpath,low,high,levels=3,amplification=20):
    t,f=load_video(video_name)
    ...
    for i in range(levels):
        ...
        print("time cost: "+str(end-star))
    recon=reconstract_from_tensorlist(filter_tensor_list)
    final=t+recon
    save_video(final,ouputpath)

if __name__=="__main__":
    low = 1.5/2.5
    high = 1.7/2.5
    start=time.time()
    magnify_color("vessel_frames.mp4","greenmagnify.mp4",low,high)
    end=time.time()
    print("time elapsed:",end-start)
    print("color magnify done")
